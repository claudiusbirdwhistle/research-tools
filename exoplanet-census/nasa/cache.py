"""SQLite response cache for NASA Exoplanet Archive TAP API."""
...
class ResponseCache:
    ...
    @staticmethod
    def _make_key(query_str):
        return hashlib.sha256(query_str.encode()).hexdigest()

    def get(self, query_str):
        key = self._make_key(query_str)
    ...
    def put(self, query_str, data, status_code=200):
        key = self._make_key(query_str)
        self.conn.execute(
