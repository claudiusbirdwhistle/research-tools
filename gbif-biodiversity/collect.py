...
from gbif.client import facet_query, get_nodes, query
...
def collect_all():
    ...
    r = facet_query("country", 300)
    save("global_country", r)
    results["global_country"] = r

    r = facet_query("year", 200)
    save("global_year", r)
    results["global_year"] = r

    r = facet_query("month", 12)
    save("global_month", r)

    r = facet_query("basisOfRecord", 20)
    save("global_basis", r)
    results["global_basis"] = r

    r = facet_query("kingdomKey", 10)
    ...
    for k, name in kingdoms.items():
        r = facet_query("year", 60, {"kingdomKey": k})
    ...
    for c, name in classes.items():
        r = facet_query("year", 60, {"classKey": c})
        class_year[str(c)] = r
    ...
    basis_year = {}
    for b in bases:
        r = facet_query("year", 60, {"basisOfRecord": b})
        basis_year[b] = r
    ...
    for i, cc in enumerate(country_codes):
        r = facet_query("year", 60, {"country": cc})
        country_year[cc] = r
    ...
    for key, filters, label in [
        ...
        r = facet_query("country", 300, filters)
        taxa_country[key] = r
    ...
    country_basis = {}
    for i, cc in enumerate(country_codes):
        r = facet_query("basisOfRecord", 10, {"country": cc})
        country_basis[cc] = r
    ...
    for cc in accum_countries:
        ...
        for ckey, cname in accum_classes.items():
            ...
            for yr in accum_years:
                r = facet_query("speciesKey", 100000, {"country": cc, "classKey": ckey, "year": yr})
    ...
    for kkey, kname in [(1, "Animalia"), (6, "Plantae")]:
        r = facet_query("classKey", 50, {"kingdomKey": kkey})
